...
class _SysOptionsMeta(type):
    ...
    def visible_languages(cls):
        return [item for item in cls.languages if item.get("visible", True)]

    @my_property(ttl=DEFAULT_SHORT_TTL)
    def visible_spj_languages(cls):
        return [item for item in cls.languages if "spj" in item and item.get("visible", True)]
